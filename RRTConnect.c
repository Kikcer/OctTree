...
void planning(coordinate_t *X_start, coordinate_t *X_end, octoTree_t *octoTree, octoMap_t *octoMap, array_t *result)
{
    ...
    result->len = 0;
    if (caldistance_int(X_start, X_end) < MIN_DISTANCE)
        return;
    ...
    for (int i = 0; i < ITER_MAX; ++i)
    {
        ...
        if (caldistance_int(&X_near_1->loc, &X_rand.loc) < 0.5 || caldistance_int(&X_near_2->loc, &X_rand.loc) < 0.5)
            continue;
        steer(&X_near_1->loc, &X_rand.loc, &X_new_1);
        steer(&X_near_2->loc, &X_rand.loc, &X_new_2);
        // printf("X_near %d,%d,%d\n",X_near_1->loc.x,X_near_1->loc.y,X_near_1->loc.z);
        // printf("X_end %d,%d,%d\n",X_end.x,X_end.y,X_end.z);
        if (obstaclefree(octoTree, octoMap, X_near_1->loc, X_new_1.loc))
        {
            X_new_1.index_parent = near_index_1;
            // printf("%d,%d,%d,", X_new_1.loc.x, X_new_1.loc.y, X_new_1.loc.z);
            ...
            if (caldistance_int(&X_new_1.loc, &X_connect_2->loc) <= MIN_DISTANCE)
            {
                X_connect_1 = &X_new_1;
                printf("%d,%d,%d,", X_connect_1->loc.x, X_connect_1->loc.y, X_connect_1->loc.z);
                break;
            }
        }
        else
        ...
        if (obstaclefree(octoTree, octoMap, X_near_2->loc, X_new_2.loc))
        {
            ...
            if (caldistance_int(&X_new_2.loc, &X_connect_1->loc) <= MIN_DISTANCE)
            {
                X_connect_2 = &X_new_2;
                break;
            }
    ...
    // printf("caldistance_int = %f , ", caldistance_int(&X_connect_1->loc, &X_connect_2->loc));
    if (caldistance_int(&X_connect_1->loc, &X_connect_2->loc) <= MIN_DISTANCE)
    {
...
short find_nearest_neighbor(coordinate_t *X_rand, array_t *current)
{
    ...
    short min = 0;
    double min_distance = caldistance_int(X_rand, &(current->arr[0].loc));
    double distance = 0;
    for (short i = 1; i < len; ++i)
    {
        distance = caldistance_int(X_rand, &(current->arr[i].loc));
...
void steer(coordinate_t *X_near, coordinate_t *X_rand, vertex_t *X_new)
{
    ...
    double length = caldistance_int(X_near, X_rand);
    X_new->loc.x = fmax(0, fmin(X_near->x + (X_rand->x - X_near->x) * (double)STRIDE / length, MAXRAND));
    X_new->loc.y = fmax(0, fmin(X_near->y + (X_rand->y - X_near->y) * (double)STRIDE / length, MAXRAND));
    X_new->loc.z = fmax(0, fmin(X_near->z + (X_rand->z - X_near->z) * (double)STRIDE / length, MAXRAND));
...
bool obstaclefree(octoTree_t *octoTree, octoMap_t *octoMap, coordinate_t start, coordinate_t end)
{
    ...
    float d = caldistance_int(&start, &end);
    ...
    while (caldistance_int(&point, &end) > MIN_DISTANCE)
    {
        probability = 3;
        point.x += dx_i;
        error_x += dif_x;
